...
# Handling Fail-Closed Warnings

If graphify reports files that left the scan corpus but still exist on disk (fail-closed), run a full re-extraction to clear it:
...
`--force` skips the incremental manifest gate and semantic cache, doing a full re-scan and re-dispatch. Use it after files are deleted, renamed, or ignore rules change — not as the default command, since it re-processes everything.

---

# Local Model Reliability

Local Ollama models can time out or return prose instead of JSON, especially under load. If extraction repeatedly fails with truncation or timeout errors:

- Increase `--api-timeout` above the 600 second default.
- Lower `--token-budget` further to keep chunks smaller.
- Confirm the model is code-tuned (e.g. qwen2.5-coder) rather than general-purpose — general models are more likely to return prose instead of structured JSON, which graphify treats as a hollow response and retries into ever-smaller chunks before eventually timing out.
...
Captured from `graphify --help` on 3 August 2026. Reference for available commands and flags.
...
1.1	3 August 2026	Switched default model to qwen2.5-coder:7b; added fail-closed and local-model reliability guidance; added full CLI reference; closed unclosed code fence.